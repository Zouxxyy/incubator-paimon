...
public final class KeyValueTableRead extends AbstractDataTableRead<KeyValue> {
    ...
    private RowType requiredRowType = null;
    ...
    private void assignValues(SplitRead<InternalRow> read) {
        ...
        read.withRequiredRowType(requiredRowType).withFilter(predicate).withIOManager(ioManager);
    }

    @Override
    public void requiredRowType(RowType requiredRowType) {
        initialized().forEach(r -> r.withRequiredRowType(requiredRowType));
