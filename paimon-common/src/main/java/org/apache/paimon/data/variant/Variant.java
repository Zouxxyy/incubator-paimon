...
import static org.apache.paimon.data.variant.VariantUtil.BINARY_SEARCH_THRESHOLD;
import static org.apache.paimon.data.variant.VariantUtil.SIZE_LIMIT;
import static org.apache.paimon.data.variant.VariantUtil.Type;
import static org.apache.paimon.data.variant.VariantUtil.VERSION;
import static org.apache.paimon.data.variant.VariantUtil.VERSION_MASK;
...
public final class Variant {
    ...
    private Variant(byte[] value, byte[] metadata, int pos) {
        this.value = value;
    ...
    public Variant copy() {
        ...
        byte[] newMetadata = Arrays.copyOf(metadata, metadata.length);
        return new Variant(newValue, newMetadata, pos);
    }
    ...
    public boolean equals(Object o) {
        ...
        final Variant that = (Variant) o;
        return Arrays.equals(this.getMetadata(), that.getMetadata())
    ...
    // access element, e.g. variant(v, $.dog.name)
    public Object variantGet(String path) {
        return VariantFunctions.variantGet(this, path);
    }
    ...
    private static void toJsonImpl(
        ...
        switch (VariantUtil.getType(value, pos)) {
            ...
            case DATE:
                appendQuoted(
                        sb, LocalDate.ofEpochDay((int) VariantUtil.getLong(value, pos)).toString());
                break;
            case TIMESTAMP:
                appendQuoted(
                        sb,
            ...
            case TIMESTAMP_NTZ:
                appendQuoted(
                        ...
                        TIMESTAMP_NTZ_FORMATTER.format(
                                microsToInstant(VariantUtil.getLong(value, pos))
            ...
            case BINARY:
                appendQuoted(
                        sb, Base64.getEncoder().encodeToString(VariantUtil.getBinary(value, pos)));
                break;
        }
    }
}
