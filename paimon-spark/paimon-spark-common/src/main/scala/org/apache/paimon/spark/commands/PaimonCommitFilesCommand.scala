...
case class PaimonCommitFilesCommand(path: Path, partitionToFileNames: Map[String, List[String]])
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    val messages =
      new SparkCommiter(partitionToFileNames.mapValues(_.asJava).asJava, fileStoreTable)
        .generatorCommitMessage()
